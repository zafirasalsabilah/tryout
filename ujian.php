...
$rujian = mysqli_fetch_array(mysqli_query($mysqli, "SELECT * FROM ujian a JOIN mapel b ON(a.id_mapel=b.id_mapel) JOIN periode c ON(a.id_periode=c.id_periode) WHERE a.id_ujian='$_GET[ujian]'"));
...
echo '<h3 class="page-header"><b>Mapel: '.$rujian['nama_mapel']. ' '.$rujian['nama_periode'].'<span class="pull-right"> Sisa Waktu: <span class="menit">'.$sisa_waktu[0].'</span> : <span class="detik"> '.$sisa_waktu[1].' </span></span></b></h3>
